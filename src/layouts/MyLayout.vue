         ...
         <div>
           ...
           <ul>
            <li v-for="friend in friends"> 
              <p> <img :src="friend.photo_50"> {{friend.first_name}} {{friend.last_name}} (День рождения: <span color="blue">{{friend.bdate}}</span>)</p>
              </li>
          </ul>
         </div>
...
export default {
  ...
  methods: {
    load(response) {
      ...
      function getFriends(){
        VK.Api.call('friends.get', {fields: ['first_name', 'last_name', 'photo_50', 'bdate'], count: "5", v: "5.8"}, 
        function(r){
        if(r.response){
          r = r.response.items;
          console.log(r);
